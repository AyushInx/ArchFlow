...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || data.errors?.[0]?.msg || 'Failed to sign up');
      }
    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-[var(--paper)] relative">
      ...
      <div className="w-full max-w-md animate-fade-in z-10">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-[var(--charcoal)] font-heading">Create Account</h1>
          <p className="text-[var(--grid-line)] mt-2">Start designing your architectures today.</p>
        </div>
        ...
        <BlueprintCard variant="light" className="bg-white">
          ...
          <p className="text-center text-sm text-[var(--grid-line)] mt-6">
            ...
            <Link href="/login" className="text-[var(--amber-signal)] font-medium hover:underline">
              Log in
            </Link>
          </p>
        </BlueprintCard>
